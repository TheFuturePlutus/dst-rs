---
description: Make a Rust crate deterministically testable — scan for determinism leaks, migrate the time seams with navian-dst-cli, hand-fix what it skips, and scaffold a seed-loop DST test.
...
seed and tested under fault injection with `navian-dst`. You drive the already-built
`navian-dst-cli` tools (`scan` + `migrate`) and hand-finish the leaks they deliberately
...
`navian-dst migrate` adds a `time` field defaulted to the *production* clock, so every
...
```bash
dst --version 2>/dev/null && DST="dst" || DST="cargo run -q -p navian-dst-cli --"
```

Everywhere below, `$DST` means `dst` (installed) or `cargo run -q -p navian-dst-cli --`.
...
the categories navian-dst-cli covers.

## Step 2 — Ensure `navian-dst` is a dependency (BEFORE migrate)

`navian-dst migrate` runs `cargo check` after rewriting and **auto-restores the
originals if the check fails** — and the rewrite references `navian_dst::Time` /
`navian_dst::ProductionTime`, so a crate without the dependency will fail that check
and silently roll back. Add it first:

```bash
# Runtime use (the migrated struct fields live in shipped code):
cargo add navian-dst --manifest-path <crate>/Cargo.toml
# If the crate should only pull navian-dst in for tests, use a dev-dependency:
# cargo add navian-dst --dev --manifest-path <crate>/Cargo.toml
```

If `cargo add` can't resolve `navian-dst` (not published / local workspace), add a
path or workspace dependency to the crate's `Cargo.toml` by hand, e.g.
`navian-dst = { path = "…/crates/navian-dst" }`. Confirm with `cargo check` before
migrating. (In *this* repo, the fixture crate at
`crates/navian-dst-cli/tests/fixtures/app` already depends on `navian-dst` — no add needed.)
...
named-field structs, it adds `time: std::sync::Arc<dyn navian_dst::Time>` to the
struct, defaults it to `navian_dst::ProductionTime` in every constructor, and
...
- **Free functions** (no `self` to hang a clock off): thread an explicit
  parameter — `fn boot_ts(time: &dyn navian_dst::Time) -> i64 { time.now_ms() }` —
  and update callers. Or move the logic onto a struct that owns a clock.
- **RANDOM leaks** (`rand::random`, `thread_rng`, `gen`/`gen_range`,
  `Uuid::new_v4`, `fastrand`): add a `rng: Arc<dyn navian_dst::Random>` struct field
  defaulted to `navian_dst::ProductionRandom`; route calls through
...
`pub fn with_time(tokens: u32, time: Arc<dyn navian_dst::Time>) -> Self`) so tests
...
`crates/navian-dst/examples/kv_chaos.rs` and the template in the skill: inject a
...
exactly like the navian-dst examples. Copy the template from the skill verbatim and

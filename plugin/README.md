# dst-init — the AI-native install layer for dst-rs
...
This layer is pure orchestration. It drives the already-built `dst-cli` tools
(`dst scan` + `dst migrate`) and the `dst-rs` library — it adds **no new Rust
...
- The `dst` CLI on `PATH`, **or** run it from this repo. Install with:
  ```bash
  cargo install --path crates/dst-cli    # provides the `dst` binary
  ```
  Without installing, the playbook falls back to `cargo run -q -p dst-cli --`.
- The target crate should depend on `dst-rs` (the playbook adds it if missing —
  it must be present before `dst migrate`, or migrate's post-rewrite
...
1. **Scan** — `dst scan --json <path>`; summarizes leak counts by category.
2. **Ensure dependency** — adds `dst-rs` to `Cargo.toml` if missing.
3. **Migrate** — `dst migrate --dry-run` then `dst migrate`; applies the
   seam-safe time rewrites (adds `time: Arc<dyn dst_rs::Time>`, defaulted to the
...
Cursor picks up the rule and follows the same playbook driving `dst scan` /
`dst migrate`, with the same conventions and seed-loop test template.

## Why there is no MCP server

MCP is for exposing a **live service** an agent calls at runtime. `dst-rs` is a
**library** and `dst-cli` is a **build-time CLI** — the agent invokes them
...
- `crates/dst-rs/README.md` — the substrate (three pillars: replay, fault
  injection, shrinking).
- `crates/dst-rs/examples/hello_retry.rs`, `examples/kv_chaos.rs` — the real

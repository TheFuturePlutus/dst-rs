//! Fixture app for the `navian-dst migrate` integration test.
//!
...
//! three rewritable TIME leaks `navian-dst migrate` v1 handles.
//!
...
//! the struct gains a `time: Arc<dyn navian_dst::Time>` field defaulted to the real
...
/// a `time: Arc<dyn navian_dst::Time>` field (which is neither `Debug` nor
